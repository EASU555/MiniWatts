...
一个用 Apple 私有 API 做的 iPhone 电池与充电信息 app。它读取手机自己的电源管理传感器——也就是 iOS 用来控制充电的那一套——显示充电器正在输出多少、其中有多少进入电芯、两者的差值（包含手机自身用电和转换损耗，不等于发热量），以及这期间手机里每一个温度传感器的读数。
...
部件温度、CPU 占用率，以及整机上传和下载速度。首页可分别选择左侧、右侧和多活动时的
读数。网速来自连续两次 Wi-Fi／蜂窝网络接口字节计数，并非 MiniWatts 自身的流量。
...
实时活动会在 MiniWatts 运行时刷新；悬浮监视器开启期间也可在后台继续采样。仅开启实时
活动不会维持后台采样。iOS 仍可能暂停、移除或结束它；不再需要后台监测时，请关闭悬浮监视器。

> **只能自签安装。** 用了私有 API，所以永远上不了 App Store，需要你自己签名安装。它只读取本地网络计数，不发起网络请求或上传数据；读到的数据不会离开你的手机。
